(function(root){
  function recommend(products,{country='JP',mode='personal',pick='share',taste='all',recipient='friend',budget=50000}={}){
    const limit=Number(budget);
    if(!Number.isFinite(limit)||limit<0)return [];
    return products.filter(p=>(p.country||'JP')===country && p.price<=limit && (mode==='quick'?p.picks.includes(pick):(taste==='all'||p.category===taste)))
